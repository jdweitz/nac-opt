...
def get_linear_bops_tf(layer, bit_width=32):
    sparsity = get_sparsity_tf(layer.kernel)
    return (
        layer.units
        * layer.input_shape[-1]
        * ((1 - sparsity) * bit_width**2 + 2 * bit_width + math.log2(layer.input_shape[-1]))
    )
...
def get_MLP_bops_tf(block, bit_width=32):
    bops = 0
    for i, layer in enumerate(block.layers):
        if isinstance(layer, tf.keras.layers.Dense):
            bops += get_linear_bops_tf(layer, bit_width)
    return bops
